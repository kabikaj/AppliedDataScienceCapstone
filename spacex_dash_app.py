...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        return px.pie(spacex_df, values='class', names='Launch Site', title='Successfull launches')
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site].groupby(['Launch Site','class']).size().reset_index(name='size')
        return px.pie(filtered_df, values='size', names='class', title=f'Successfull launches in site {entered_site}')
...
def get_pie_chart(entered_site, payload):
    ...
    # values Payload Mass (kg) and class for the selected site, and color-label the point using Boosster Version Category likewise. 
        ...
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site] #.groupby(['Payload Mass (kg)','class']).size().reset_index(name='size')
